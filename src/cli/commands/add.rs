use anyhow::Result;
use std::fs;
use crate::{core::{Project, manifest::Manifest}, package::manager::PackageManager};
...
pub fn execute(p:&Project, package: &str) -> Result<()> {
    ...
    //     anyhow::bail!("Package management is disabled. Enable it in ppargo.toml:\n[features]\npackages = true")
    // }
    ...
    // // Save manifest
    // let toml_str = toml::to_string(&p.manifest)?;
